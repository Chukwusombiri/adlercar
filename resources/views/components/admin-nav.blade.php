        ...
        <li class="nav-item dropdown">
          <a class="nav-link" data-toggle="dropdown" href="#">
            ...
            @if (count($latest_requests)>0)
              <span class="badge badge-warning navbar-badge">{{count($latest_requests)}}</span>         
            @endif
          </a>
          <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
              @if (count($latest_requests)>0)
                  <span class="dropdown-item dropdown-header">{{count($latest_requests)}} New Shipment request</span>                      
                    <a href="{{route('admin.shipments')}}" class="flex flex-nowrap justify-center dropdown-item">                           
                      <span>VIEW ALL</span>
                    </a>                                          
              @else
                <a href="#" class="dropdown-item dropdown-footer">No new shipment request</a>                     
              @endif                    
          </div>
        </li>
